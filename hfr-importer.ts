...
async function add_new_totoz(candidates: CandidateType[], c: ClientBase) {
  ...
  for (const r of rows) {
    const img = await get_totoz_image(
      candidates.find((t) => t.name == r.name)!.url,
    );

    images.push({
      name: r.name,
      image: img ? toPgBytea(Buffer.from(img)) : null,
    });
  }
...
async function update_tags(totoz: CandidateType[], c: ClientBase) {
  ...
  if (rows.length > 0)
    console.log(
      rows.map((r: any) => "Added: " + r.totoz_name + ":" + r.name).join("\n"),
    );
}
