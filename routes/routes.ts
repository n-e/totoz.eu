...
routes.post(
  ...
  throwtonext(async (req, res, next) => {
    ...
    for (const t of tags)
      if (!t.match(/^[A-Za-z0-9-_]+$/)) errors.push(`Tag '${t}' is invalid`);
...
async function validate_totoz_name(name: string) {
  const results = await pool.query("select name from totoz where name = $1", [
    name,
  ]);
